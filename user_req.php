...
$rdate=date("d-m-Y");
 if(isset($btn))
	...
	$mq=mysql_query("select max(id) from eb_request");
    $mr=mysql_fetch_array($mq);
    $id = $mr['max(id)']+1;
	
	$n=mysql_query("insert into eb_request(id,uname,month,year,rdate,status) values($id,'$uname','$month','$year','$rdate','0')");
                 
	 ?>
	 <script language="javascript">
	 window.location.href="user_req.php?act=success";
	 </script>
	 <?php
...
<div class="row">
	...
	<input type="submit" name="btn" value="Send Request" class="btn btn-primary" onClick="return validate()">
                   </div>
                  </form>
...
<!--end content area-->
  ...
  <div align="center">
	...
	$qry=mysql_query("select * from eb_request where uname='$uname'");
    ...
    <table width="95%" border="1">
      <tr>
        ...
        <th width="209" class="bg1" scope="col">Status</th>
      </tr>
      <?php
	  $i=0;
      ...
      <tr>
		...
		if($row['status']=="1")
		{
		echo '<a href="view_det.php?rid='.$row['id'].'">View</a>';
		}
		else
		{
		echo "Request Sent..";
